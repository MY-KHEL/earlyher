import { Activity, ArrowRight, Clock, MapPin, Microscope, Play, PlayCircle } from "lucide-react";
      ...
      <div className="flex flex-col gap-8 w-full ">
        <div className="flex flex-col ">
          <div className="flex gap-2 items-center bg-[#3c8e65] w-fit rounded-full px-3 py-2">
            <div className="bg-[#d81b60] p-2 rounded-full"><Microscope color="white" size={16}/></div>
        ...
        <p className="max-w-[400px] text-white/70">
          ...
          perspiciatis debitis, iusto quod iste consequatur beatae aspernatur
          soluta quidem numquam aliquam aperiam minima, voluptatem odio.
        </p>
        ...
        <div className="flex max-md:flex-col items-center gap-10 ">
          ...
          <Link href={'/'} className="flex items-center text-white/70 gap-4"> <PlayCircle size={30}/> Watch How it Works </Link>
        </div>
      </div>
      <div className=" flex flex-col gap-4  w-full items-end">
          {details.map((detail,index)=>(
            <StatsCard icon={detail.icon} key={index} iconBackground={detail.iconBackground} heading={detail.heading} content={detail.content}/>
          ))}
      </div>
    </section>
  );
...
const StatsCard =({icon,iconBackground,heading,content}:IStatsCard)=>(
  <div className="bg-[#3c8e65] rounded-2xl w-full md:max-w-[400px] p-4 flex gap-4 items-center">
    <div className={`p-3 rounded-2xl  bg-${iconBackground}`}>
      {icon}
